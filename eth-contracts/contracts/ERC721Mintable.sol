...
contract Ownable {
    ...
    //  create an internal constructor that sets the _owner var to the creator of the contract
    ...
    // create an event that emits anytime ownerShip is transferred (including in the constructor)

    function transferOwnership(address newOwner) public onlyOwner {
        // add functionality to transfer control of the contract to a newOwner.
        // make sure the new owner is a real address
        require(newOwner != address(0), "Address provided must be valid");
        _owner = newOwner;
        emit ownerShip(_owner);
    }
}
...
contract Pausable is Ownable {
    ...
    function setPauseStatus(bool pauseStatus) public onlyOwner {
        require(_paused != pauseStatus, "Contract pause status already in this state");
        _paused = pauseStatus;
        if (_paused){
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != tokenOwner, "given address is owner of token");

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == tokenOwner || isApprovedForAll(tokenOwner, msg.sender), "caller must either be owner or have approval");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "from address must be the token owner");

        // require token is being transfered to valid address
        require(to != address(0), "destination address must be valid");
...
contract RealtyERC721Token is ERC721Metadata("Realty", "RLT", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
